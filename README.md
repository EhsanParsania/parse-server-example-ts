...
* `npm run dev` will start both the Parse Server with auto-restart and the TypeScript compiler in watch mode.
* `npm run build` will compile all TypeScript files from `cloud-ts` to JavaScript in the `cloud` directory.
* `npm run ts-watch` will start the TypeScript compiler in watch mode, automatically compiling files when they change.
* `npm run ts-check` will check TypeScript files for errors without compiling them.
...
This project includes support for writing cloud code in TypeScript. TypeScript files are stored in the `cloud-ts` directory and are automatically compiled to JavaScript in the `cloud` directory.

To get started with TypeScript:
1. Write your cloud code in the `cloud-ts` directory
2. Run `npm run dev` to start both the Parse Server and TypeScript compiler
3. Any changes to TypeScript files will be automatically compiled to JavaScript
4. See the `cloud-ts/README.md` file for more details on the TypeScript workflow
